...
func createAttrUser(ctx context.Context, t *testing.T, deps authusecases.AdminUserDeps) *spec.User {
	t.Helper()
...
func TestUpdateUserAcceptsBuiltinAttribute(t *testing.T) {
	...
	updated, err := authusecases.UpdateUser(ctx, deps, authusecases.UpdateUserInput{
...
func TestUpdateUserRejectsUndefinedAttribute(t *testing.T) {
	...
	_, err := authusecases.UpdateUser(ctx, deps, authusecases.UpdateUserInput{
...
func TestUpdateUserAcceptsTenantCustomAttribute(t *testing.T) {
	...
	if _, err := authusecases.UpdateUser(ctx, deps, authusecases.UpdateUserInput{
		ActorSub: "admin", Sub: user.Sub, Attributes: &bad, Now: time.Now().UTC(),
	}); !errors.Is(err, authusecases.ErrInvalidAttribute) {
