/**
 * CLI argument parsing for scl-to-jsonschema. Pure — no IO, no exits.
 */
...
const USAGE = [
  'Usage: scl-to-jsonschema --scl <file> [--out <path>]',
  '       scl-to-jsonschema --help',
  '',
  '  --scl   SCL YAML document (a context map or a single context) (required).',
  '  --out   Output JSON Schema path. Without --out the schema is written to stdout.',
  '',
].join('\n')
