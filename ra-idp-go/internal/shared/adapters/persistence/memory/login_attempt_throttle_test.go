...
func TestLoginAttemptThrottleLocksAndExpires(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		result, err := throttle.RecordFailure(ctx, authnports.LoginThrottleAccount, "alice", now)
	...
	acquire, err := throttle.TryAcquire(ctx, authnports.LoginThrottleAccount, "alice", now)
	if err != nil || acquire.Allowed || acquire.RetryAfterSeconds != 120 {
		t.Fatalf("acquire result=%+v err=%v", acquire, err)
	}
	acquire, err = throttle.TryAcquire(ctx, authnports.LoginThrottleAccount, "alice", now.Add(121*time.Second))
...
func TestLoginAttemptThrottleSuccessClearsAccountOnly(t *testing.T) {
	...
	ctx := context.Background()
	now := time.Now().UTC()
	_, _ = throttle.RecordFailure(ctx, authnports.LoginThrottleAccount, "alice", now)
	_, _ = throttle.RecordFailure(ctx, authnports.LoginThrottleIP, "203.0.113.1", now)
	if err := throttle.RecordSuccess(ctx, authnports.LoginThrottleAccount, "alice"); err != nil {
		t.Fatal(err)
	}
	account, _ := throttle.TryAcquire(ctx, authnports.LoginThrottleAccount, "alice", now)
	ip, _ := throttle.TryAcquire(ctx, authnports.LoginThrottleIP, "203.0.113.1", now)
