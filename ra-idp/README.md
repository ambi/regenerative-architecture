...
├── decisions/            ← Layer 2: コンセプション・ベースライン / ADR
...
保存対象は `spec/scl.yaml` ・ `decisions/CONCEPTION.md` ・
`decisions/CONCEPTION_BASELINE.md` ・ `decisions/ADR-*.md` ・ `infra/migrations/` ・
...
| パスワードポリシー カスタマイズ | テナント別のポリシー上書き（history depth / min length / max length / common-password dictionary 等）。Phase 0 で固定値として置いた `PASSWORD_POLICY` をテナント解決ポートに置き換える |
