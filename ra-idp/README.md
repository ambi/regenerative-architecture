...
| 領域 | 不足している機能 |
| ---- | ---------------- |
| ユーザー認証 | MFA/WebAuthn、パスキー、アカウント復旧、ログイン試行レート制限、Argon2/bcrypt など本番用 password hashing |
| セッション管理 | セッション一覧・失効 UI、デバイス管理、step-up authentication、`acr_values` に応じた認証手段選択 |
...
### OAuth2/OIDC IdP コンポーネント

| 領域 | 不足している機能 |
| ---- | ---------------- |
| OIDC セッション | `id_token_hint` の署名検証・client 解決、Back-Channel Logout、Front-Channel Logout、Session Management |
