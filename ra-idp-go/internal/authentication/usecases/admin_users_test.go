...
func TestDeleteUserAnonymizesAndCascades(t *testing.T) {
	...
	if err := authusecases.DeleteUser(ctx, deps, authusecases.DeleteUserInput{
...
func TestDeleteUserRejectsSelfDelete(t *testing.T) {
	...
	err := authusecases.DeleteUser(ctx, authusecases.AdminUserDeps{UserRepo: userRepo},
		authusecases.DeleteUserInput{ActorSub: "admin-1", Sub: "admin-1", Now: now})
	if !errors.Is(err, authusecases.ErrSelfDeleteForbidden) {
