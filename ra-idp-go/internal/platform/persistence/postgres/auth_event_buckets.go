...
func (s *AuthEventBucketStore) Record(
	...
	nowUTC := now.UTC()
	windowStart := nowUTC.Truncate(authports.AuthEventBucketWindow)
	var (
		count     int64
		firstSeen time.Time
		lastSeen  time.Time
...
RETURNING count, first_seen, last_seen, (xmax = 0)`,
	...
	if err != nil {
		return authports.AuthEventBucketResult{}, err
	}
	return authports.AuthEventBucketResult{
...
LIMIT $2`, tenantID, limit)
	...
	defer rows.Close()
	out := []authports.AuthEventBucket{}
	for rows.Next() {
		var (
			b     authports.AuthEventBucket
			kind  string
			count int64
		)
		if err := rows.Scan(
		...
		b.Kind = authports.AuthEventBucketKind(kind)
		b.Count = int(count)
