...
4. **Claim release は protocol-neutral capability として `ClaimMapping` に置く。**
...
6. **管理 UI の「アプリケーション」は将来 `Application` の語彙に予約する。**
   Application は OIDC client / SAML SP / WS-Fed RP を束ねる上位 aggregate を所有し、
   表示名、所有者、ライフサイクル、割当、共通監査、共通ポリシーを扱う。各 protocol context は
   application に紐づく protocol binding を所有する。

7. **Application が実装されるまで、プロトコル固有画面は正確な名前にする。**
   現在の OAuth2/OIDC client 管理画面を単に「アプリケーション」と呼ぶのは避ける。
   統合 Application が入った時点で、上位ナビゲーション名を「アプリケーション」に戻し、
...
- `internal/federation` は `internal/wsfederation` へ移す。ただし protocol-neutral な
  claim release / signing key lifecycle の最終配置は Application / Saml 導入時に見直す。
- [[ADR-059]] は「WsFederation bounded context 導入」の決定としては置き換え対象になる。
  claim mapping を宣言的にし fail-closed にする決定は維持する。
- OAuth2/OIDC の ID Token / UserInfo claim 選択は、将来 `ClaimMapping` を再利用する候補にする。
...
- [[wi-29-saml2-idp]] の UI scope は「admin clients に SAML client/SP 種別を追加」ではなく、
  Application または暫定 SAML 専用画面との整合を取る必要がある。
