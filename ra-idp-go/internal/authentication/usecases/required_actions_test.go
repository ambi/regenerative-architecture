...
func newRequiredActionFixture(t *testing.T) (context.Context, authusecases.AdminUserDeps, *[]spec.DomainEvent, *spec.User) {
	t.Helper()
...
func TestSetAndClearUserRequiredAction(t *testing.T) {
	...
	updated, err := authusecases.SetUserRequiredAction(
	...
	updated, err = authusecases.ClearUserRequiredAction(
		ctx, deps, "admin", user.Sub, spec.RequiredActionUpdatePassword, now,
	)
	if err != nil {
		t.Fatal(err)
...
func TestSetUserRequiredActionRejectsUnknownAction(t *testing.T) {
	...
	_, err := authusecases.SetUserRequiredAction(
...
func TestChangePasswordAutoClearsUpdatePasswordAction(t *testing.T) {
	...
	if _, err := authusecases.SetUserRequiredAction(
		ctx, deps, "admin", user.Sub, spec.RequiredActionUpdatePassword, now,
	); err != nil {
		t.Fatal(err)
	}
