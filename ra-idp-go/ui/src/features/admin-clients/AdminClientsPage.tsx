...
export function AdminClientsPage({
  ...
  const [issuedSecret, setIssuedSecret] = useState<{ clientID: string; secret: string } | null>(
    null,
  )
  ...
  async function refresh(preferredID = selectedID) {
    ...
    setClients(next)
    setSelectedID(
      next.find((client) => client.client_id === preferredID)?.client_id ??
        next[0]?.client_id ??
        '',
    )
  }
    ...
    <>
      ...
      >
        <section className="grid gap-3 sm:grid-cols-3" aria-label="OAuth2/OIDC クライアント概要">
          ...
          <Metric
            label="機密タイプ"
            value={clients.filter((client) => client.client_type === 'confidential').length}
          />
          <Metric
            label="PAR 必須"
            value={clients.filter((client) => client.require_pushed_authorization_requests).length}
          />
        </section>
          ...
          <div className="flex flex-col gap-3 border-b border-slate-200 p-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="relative w-full max-w-xl">
              ...
              <Input
                ...
                placeholder="名前、client ID、redirect URI、scope で検索"
                aria-label="OAuth2/OIDC クライアントを検索"
              />
            </div>
            <Button
              variant="outline"
                  ...
                  {filtered.map((client) => (
                    <tr
                      ...
                      className={cn(
                        ...
                        selectedID === client.client_id && 'bg-blue-50/60 hover:bg-blue-50/80',
                      )}
                    >
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex flex-1 flex-col gap-5 p-5">
        ...
        <Detail
          ...
          value={
            [
              client.require_pushed_authorization_requests ? 'PAR required' : '',
              client.dpop_bound_access_tokens ? 'DPoP bound' : '',
            ]
              .filter(Boolean)
              .join(', ') || '標準'
          }
...
function ClientFormDialog({
  ...
  return (
    <Dialog
      title={mode === 'create' ? 'クライアントを追加' : 'クライアントを編集'}
      onClose={onClose}
    >
      <form onSubmit={onSubmit}>
        <div className="grid max-h-[70vh] gap-5 overflow-y-auto p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="種別">
                <Select
                  ...
                  onChange={(value) => {
                    const type = value as ClientForm['client_type']
                    set('client_type', type)
                    set(
                      'token_endpoint_auth_method',
                      type === 'public' ? 'none' : 'client_secret_basic',
                    )
                  }}
              ...
              <Field label="認証方式">
                <Select
                  ...
                  onChange={(value) =>
                    set(
                      'token_endpoint_auth_method',
                      value as AdminClient['token_endpoint_auth_method'],
                    )
                  }
                  options={[
                    'client_secret_basic',
          ...
          <Field label="Redirect URI" hint="1行に1 URI">
            <Textarea
              required
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Grant types" hint="カンマ区切り">
              <Input
                required
                value={form.grant_types}
                onChange={(event) => set('grant_types', event.target.value)}
              />
            </Field>
            <Field label="Response types" hint="カンマ区切り">
              <Input
                value={form.response_types}
                onChange={(event) => set('response_types', event.target.value)}
              />
            </Field>
          </div>
          <Field label="Scope" hint="空白区切り">
            <Input value={form.scope} onChange={(event) => set('scope', event.target.value)} />
          </Field>
          {mode === 'create' && form.token_endpoint_auth_method === 'private_key_jwt' && (
            <Field label="JWKS URI">
              <Input
                type="url"
                required
                value={form.jwks_uri}
                onChange={(event) => set('jwks_uri', event.target.value)}
              />
            </Field>
          )}
          {mode === 'create' && form.token_endpoint_auth_method === 'tls_client_auth' && (
            <Field label="TLS client certificate Subject DN">
              <Input
                required
                value={form.tls_client_auth_subject_dn}
                onChange={(event) => set('tls_client_auth_subject_dn', event.target.value)}
              />
            </Field>
          )}
          <Check
            label="PAR を必須にする"
            checked={form.require_pushed_authorization_requests}
            onChange={(value) => set('require_pushed_authorization_requests', value)}
          />
          <Check
            label="DPoP bound access token を要求する"
            checked={form.dpop_bound_access_tokens}
            onChange={(value) => set('dpop_bound_access_tokens', value)}
          />
        </div>
    ...
    <Dialog title="クライアントを削除" onClose={onClose}>
      <div className="p-6">
        <div className="flex gap-3 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-900">
          ...
          <p>
            削除後、この client ID
            を使う新しい認可・トークン要求は失敗します。参照中のデータがある場合、削除は拒否されます。
          </p>
        </div>
...
function SecretDialog({
  ...
  return (
    <Dialog title="Client secret を保存してください" onClose={onClose}>
      <div className="p-6">
        <Alert>
          この secret はこの画面を閉じると再表示できません。安全な保管先へ保存してください。
        </Alert>
        ...
        <div className="mt-1 flex gap-2">
          <code className="min-w-0 flex-1 break-all rounded-lg bg-slate-950 p-3 text-sm text-white">
            {value.secret}
          </code>
          <Button variant="outline" onClick={() => void copy()} aria-label="secretをコピー">
            <IconCopy size={17} />
            {copied ? 'コピー済み' : 'コピー'}
          </Button>
        </div>
      ...
      <div className="flex justify-end border-t border-slate-200 bg-slate-50 px-6 py-4">
        <Button onClick={onClose}>保存しました</Button>
      </div>
    </Dialog>
  )
}
...
function Detail({ label, value, mono = false }: { label: string; value: string; mono?: boolean }) {
  return (
    <div>
      ...
      <p
        className={cn(
          'mt-1 whitespace-pre-wrap break-all text-sm text-slate-900',
...
function Select({
  ...
  return (
    <select
      ...
      className="h-12 w-full rounded-lg border border-slate-300 bg-white px-3.5 text-sm outline-none focus:border-blue-600 focus:ring-3 focus:ring-blue-600/10"
    >
      {options.map((option) => (
...
function Textarea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return (
    <textarea
      {...props}
      className="min-h-24 w-full rounded-lg border border-slate-300 bg-white px-3.5 py-3 text-sm outline-none focus:border-blue-600 focus:ring-3 focus:ring-blue-600/10"
    />
  )
}
...
function Check({
  ...
  return (
    <label className="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm font-medium">
      <input
        type="checkbox"
        checked={checked}
