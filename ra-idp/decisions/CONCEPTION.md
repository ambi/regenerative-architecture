...
- TypeScript / Bun は現在の実装手段であり、仕様核には含めない。
...
- `bun test`、`bun run typecheck`、`bun run lint` が成功する。
- `bun run gen:all` 後に生成物の差分が残らない。
- `bun run check:coherence` が SCL 内部参照と派生物の整合を確認する。
