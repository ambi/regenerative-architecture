...
| DPoP replay cache / access token denylist / client assertion replay | `jti` / token hash | 値が global 一意なので衝突なし。Valkey namespace は per-tenant に分ける (項 5) |
...
### 5. Valkey key namespace

すべての Valkey ストアで key の先頭に `tenant:{id}:` を付与する。
影響を受けるストア:
...
- migration `0008_tenant_pk_recompose.sql` で PK / unique / FK 再構成、
  Valkey key prefix 切替、cross-tenant guard を入れる (PR-B)。
