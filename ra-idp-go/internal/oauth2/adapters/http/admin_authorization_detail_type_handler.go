...
func (d Deps) handleListAuthorizationDetailTypes(c *echo.Context) error {
	...
	return core.NoStoreJSON(c, http.StatusOK, map[string]any{"types": out})
}
...
func (d Deps) handleGetAuthorizationDetailType(c *echo.Context) error {
	...
	t, err := d.AuthzDetailTypeRepo.FindByType(c.Request().Context(), core.RequestTenantID(c), c.Param("type"))
	if err != nil {
		return err
	}
	if t == nil {
		return core.WriteBrowserError(c, http.StatusNotFound, "type_not_found", "authorization_details type が存在しません")
	}
	return core.NoStoreJSON(c, http.StatusOK, toAuthorizationDetailTypeResponse(t))
}
...
func (d Deps) handleCreateAuthorizationDetailType(c *echo.Context) error {
	...
	if strings.TrimSpace(req.Type) == "" {
		return core.WriteBrowserError(c, http.StatusBadRequest, "invalid_request", "type が必要です")
	}
	...
	if existing != nil {
		return core.WriteBrowserError(c, http.StatusConflict, "type_exists", "同名の type が既に存在します")
	}
	now := time.Now().UTC()
	state := req.State
	if state == "" {
	...
	return core.NoStoreJSON(c, http.StatusCreated, toAuthorizationDetailTypeResponse(t))
}
...
func (d Deps) handleUpdateAuthorizationDetailType(c *echo.Context) error {
	...
	return core.NoStoreJSON(c, http.StatusOK, toAuthorizationDetailTypeResponse(existing))
}
...
func (d Deps) handleDeleteAuthorizationDetailType(c *echo.Context) error {
	...
	if err := d.AuthzDetailTypeRepo.Delete(c.Request().Context(), core.RequestTenantID(c), c.Param("type")); err != nil {
		return err
	}
...
func (d Deps) saveValidatedType(c *echo.Context, t *spec.AuthorizationDetailType) error {
	if err := t.Validate(); err != nil {
		return core.WriteBrowserError(c, http.StatusBadRequest, "invalid_type", err.Error())
	}
	for _, rule := range t.Schema.Rules {
		if !rule.Semantics.Valid() {
			return core.WriteBrowserError(c, http.StatusBadRequest, "invalid_type", "未知の field semantics: "+string(rule.Semantics))
		}
	}
