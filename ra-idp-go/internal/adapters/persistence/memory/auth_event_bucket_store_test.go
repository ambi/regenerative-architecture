...
func TestAuthEventBucketStoreAggregatesWithinWindow(t *testing.T) {
	...
	first, err := store.Record(ctx, authports.AuthEventBucketFailedLogin, "acme", "k1", base)
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := 2; i <= 5; i++ {
		res, err := store.Record(ctx, authports.AuthEventBucketFailedLogin, "acme", "k1", base.Add(time.Duration(i)*time.Second))
		if err != nil {
			t.Fatal(err)
		}
...
func TestAuthEventBucketStoreNewWindowIsSeparate(t *testing.T) {
	...
	if _, err := store.Record(ctx, authports.AuthEventBucketFailedLogin, "acme", "k1", base); err != nil {
		t.Fatal(err)
	}
	// 5 分窓の外なら別 bucket・再び FirstInWindow。
	next := base.Add(authports.AuthEventBucketWindow)
	res, err := store.Record(ctx, authports.AuthEventBucketFailedLogin, "acme", "k1", next)
	if err != nil {
		t.Fatal(err)
	}
	if !res.FirstInWindow {
