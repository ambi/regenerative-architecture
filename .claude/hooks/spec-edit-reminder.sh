...
case "$fp" in
  *scl.yaml|*/spec/contexts/*.yaml)
    msg='SCL を編集しました。`cd tools && bun run yaml-check:scl` で検証し、`just scl-render` で派生物（HTML / JSON Schema / OpenAPI）を再生成してください（drift を残さない）。scl.yaml に wi/ADR/commit 番号は書かないこと。'
    ;;
  work-items/*.yaml|*/work-items/*.yaml)
    msg='work-item を編集しました。`cd tools && bun run yaml-check:work-items` で検証してください。書式の正本は CHANGE_RECORD_FORMAT.md §1。完了・中止にしたファイルは work-items/done/ へ移すこと。'
    ;;
