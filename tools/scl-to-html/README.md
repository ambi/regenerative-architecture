...
work-item / completion-report (`changes/<id>/*.yaml`) も同じページから辿れる。
...
- `--scl` のみ必須。`--decisions` / `--changes` 省略時はそのタブが空表示になる。
...
| Changes   | work-item.yaml と completion-report.yaml (status 順インデックス) |
...
- work-item / completion-report は同 `yaml-check/schemas/`
...
src/
  ...
  load.ts              # SCL / decisions / changes ローダ (file IO のみ)
  ...
  render-changes.ts    # work-item + completion-report の HTML 化
...
以降「SCL / ADR / 変更履歴 を一緒に読みたい」という運用上の要請が増え、
モジュール分割と Decisions / Changes タブを追加した。詳細は
