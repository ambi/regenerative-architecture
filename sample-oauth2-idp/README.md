...
`SPECIFICATION_CORE_LANGUAGE.md` を参照）。本 README はサンプルの構成と
実行方法、および本サンプル固有の意思決定（ADR）の索引を扱う。
...
```bash
docker build -f infra/docker/Dockerfile -t oauth2-idp:dev .
...
### CI / CD (`.github/workflows/oauth2-idp-*.yaml`)

| ワークフロー   | トリガー            | 内容                                                                                |
| -------------- | ------------------- | ----------------------------------------------------------------------------------- |
| `ci.yaml`      | PR / push to main   | typecheck + 全テスト + 派生物 drift 検知 + Trivy + CodeQL                           |
| `nightly.yaml` | cron 03:00 JST      | k6 負荷試験 (SLO 違反で fail)                                                       |
| `release.yaml` | tag `oauth2-idp-v*` | multi-arch build → cosign 署名 → CycloneDX SBOM 添付 → SLSA Level 3 provenance 生成 |

---

## 意思決定の索引（ADR）

サンプル固有の意思決定はすべて `decisions/` の ADR にある。RA / SCL の採用そのものに
関する選択は本サンプルでは前提として扱い、ADR として残していない。
