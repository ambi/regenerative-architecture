...
- **claim 露出**: `visibility=claim_exposed` の属性のみ UserInfo に出る。開示は
  要求 scope で gating する (`profile` / `phone` / `address`、scope を持たない
  custom 属性は `custom_attribute`)。`address_*` は §5.1.1 の入れ子 `address`
  オブジェクトへ再構成する (`spec.ClaimsForScopes`)。ID Token への展開は後続 PR。
