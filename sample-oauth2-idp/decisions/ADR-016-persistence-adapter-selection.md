...
1. **`src/usecases/*` と `adapters/http/*` を 1 行も変更せずに永続化を入れ替えられる**
...
- **DynamoDB / ScyllaDB**: 単一テーブルでの水平スケールは優秀だが、
  ...
  本サンプルの規模 (RPS 5000) では Postgres で十分。
...
- **ORM (Drizzle / Prisma) を使う**: 「永続化アダプタが差し替え可能」であることを
  示すサンプルにおいて、ORM 抽象が SQL の表現力を制限すると、
  RFC 9700 で要求される `SELECT FOR UPDATE` の正確な制御が阻害される。
  本サンプルは `pg` (node-postgres) を直接使い、SQL を可視化する。

## 影響

- 新規依存: `pg`, `ioredis`, `kafkajs`
- 新規ポート: `src/ports/event-sink.ts` `src/ports/transaction.ts`
...
- `src/usecases/*` と `adapters/http/*` と `spec/*` は **完全に無変更**
