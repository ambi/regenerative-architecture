...
1. **エンドポイント**
   ...
   - `GET /device` / `POST /device` (§3.3): verification_uri。ユーザーが user_code を入力し
     承認 / 拒否する。ユーザー認証は本サンプルでは X-User-Sub（authorize と同方針）。
...
- user_code は低エントロピーなので、verification_uri での入力レート制限が production では必須
  （本サンプルは枠組みのみ）。`DeviceAuthorizationDenied` の多発を SIEM で総当たり検知できる
  よう監査イベントを設計した。
...
- **承認画面を 2 ステップ (enter_user_code → approve) に分割**: サンプルの UX を単純化し、
